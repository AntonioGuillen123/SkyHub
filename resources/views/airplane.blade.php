                                                ...
                                                <div x-show="openFlight" x-collapse
                                                    ...
                                                    <ul class="space-y-1">
                                                        @forelse ($users as $user)
                                                            ...
                                                            @endphp
                                                            <li
                                                                class="p-2 bg-white shadow-sm rounded-md flex justify-between">
                                                                <span>👤 {{ $userName }}</span>
                                                                <span>📧 {{ $userEmail }}</span>
                                                                <span class="text-sm text-gray-500">📅
                                                                    {{ $bookingDate }}</span>
                                    ...
                                    <ul class="space-y-2">
                                        @forelse ($flights as $flight)
                                            ...
                                            <li x-data="{ openFlight: false }" class="p-2 bg-white shadow-sm rounded-md">
                                                ...
                                                <div x-show="openFlight" x-collapse class="bg-gray-50 p-2 mt-2 rounded-lg">
                                                    <div class="text-gray-800 font-semibold">👥 Passengers:</div>
                                                    <ul class="space-y-1">
                                                        @forelse ($users as $user)
                                                            @php
                                                                $userName = $user->name;
                                                                $userEmail = $user->email;
                                                                $bookingDate = date('Y-m-d H:i', strtotime($user->pivot->created_at));
                                                            @endphp
                                                            <li class="p-2 bg-white shadow-sm rounded-md flex justify-between">
                                                                <span>👤 {{ $userName }}</span>
                                                                <span>📧 {{ $userEmail }}</span>
                                                                <span class="text-sm text-gray-500">📅 {{ $bookingDate }}</span>
                                                            </li>
                                                        @empty
                                                            <li class="p-2 text-gray-500">No passengers on this flight.</li>
                                                        @endforelse
                                                    </ul>
                                                </div>
                                            </li>
